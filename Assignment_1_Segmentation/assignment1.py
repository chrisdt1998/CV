...
points = sio.loadmat("/Users/chris/Documents/GitHub/CV/Assignment_1_Segmentation/data/pts.mat")["data"].T
print(points.shape)
#points = color.rgb2lab(points)


class Segmentation():
	def __init__(self, data, r, threshold, c):
		self.data = data
		self.r = r
	...
	def findpeak(self, idx):
		...
		while True:
			...
			if spatial.distance.cdist(centre, np.average(points_in_radius, axis=0).reshape(1,-1), metric='euclidean') < self.threshold:
				self.peaks[idx] = np.average(points_in_radius, axis=0)
				break
	...
	def findpeak_opt(self):
		...
		while True:
			...
			# Check if we have reached the threshold and return the peak
			if spatial.distance.cdist(centre, np.average(points_in_radius, axis=0).reshape(1,-1), metric='euclidean') < 0.01:
				self.peaks[idx] = np.average(points_in_radius, axis=0)
...
try1 = Segmentation(points, 2, 0.01, 4)
try1.run()
